...
if __name__ == "__main__":
    ...
    parser.add_argument('--baseroot', type=str, default='/data/DIV2K/DIV2K_valid_HR',
                        help='root')
    parser.add_argument('--num', type=int, default=10, help='number of sample images')
    ...
    if opt.range == '0255':
        ...
        noise = np.random.normal(loc=0.0, scale=opt.noise_scale, size=img.shape)
        img_noise = img + noise
        current_psnr, l2, l1 = estimate(img_noise, img, 255)
        print('PSNR:', current_psnr)
        print('L2 Loss:', l2)
        print('L1 Loss:', l1)
        img_noise = np.clip(img_noise, 0, 255).astype(np.uint8)
